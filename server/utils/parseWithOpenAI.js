/**
 * Generic credit card statement parser powered by OpenAI.
 *
 * Uses gpt-4o-mini by default — very cheap (~$0.15 / 1M input tokens),
 * covered by free-tier credits for new OpenAI accounts, and supports
 * strict JSON mode.
 *
...
Schema:
{
  "issuer":         string | null,   // Bank / card-issuer name, e.g. "HDFC", "Chase", "Emirates NBD", "American Express"
  "issuerFull":     string | null,   // Full legal name if visible, else same as issuer
  "cardNetwork":    string | null,   // "VISA" | "MASTERCARD" | "AMEX" | "RUPAY" | "DISCOVER" | "DINERS" | null
  "currency":       string | null,   // ISO-4217: "INR", "AED", "USD", "EUR", "GBP", "SGD", "SAR" ...
  "cardLast4":      string | null,   // last 4 digits if visible, else null
  "cardLast2":      string | null,   // last 2 digits when ONLY last 2 are visible (e.g. RBL, SBI masked cards)
  "cardholderName": string | null,
  "billingCycle":   string | null,   // "DD Mon YYYY - DD Mon YYYY"
  "statementDate":  string | null,   // "DD Mon YYYY"
  "dueDate":        string | null,   // "DD Mon YYYY"
  "totalAmount":    number | null,   // amount only, no currency symbol
...
3. Currency:
   ...
   - S$ / SGD                   → SGD
   - SAR / ر.س                  → SAR
   - Otherwise infer ISO 4217 code from label text.
...
5. Amount labels (use these, in priority order):
...
6. Dates — normalize to "DD Mon YYYY" (e.g. "07 Sep 2025").
   ...
   (d) Disambiguation guard: if the FIRST number is greater than 12 it must
       be the day; prefer day-first ordering in all cases.
...
7. Due date must come from a label containing "Due Date" / "Payment Due".
   Never derive it from statement date or billing-cycle end.

8. If a field is truly not present or ambiguous → return null (do NOT guess).
